    ...
    while completed < n:
        ...
        if available_processes:
            ...
            if process.burst_time == 0:
                ...
                process.waiting_time = process.turnaround_time - (process.burst_time + process.turnaround_time)
                completed += 1
            current_time += 1
        else:
            current_time += 1
...
def main():
    file_path = 'processes.xlsx'
    output_file_path_sjf_preemptive = 'sjf_preemptive_output.xlsx'
    # Read processes
    processes = read_processes_from_excel(file_path)
    
    # SJF Preemptive
    sjf_preemptive(processes)
    save_processes_to_excel(processes, output_file_path_sjf_preemptive)
